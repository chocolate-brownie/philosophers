...
/*   Updated: 2025/04/07 02:29:41 by mgodawat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	thinking(t_philo *philo)
{
	...
	if (philo->data->nbr_of_phils % 2 == 0)
...
int	eat_with_fork_right(t_philo *philo)
{
	...
	philo->data->fork_id = right_fork;
	print_message(philo, FORK_ONE);
	...
	philo->data->fork_id = left_fork;
	print_message(philo, FORK_TWO);
	eating(philo);
	pthread_mutex_unlock(philo->data->mtx_fork + right_fork);
	pthread_mutex_unlock(philo->data->mtx_fork + left_fork);
	return (0);
